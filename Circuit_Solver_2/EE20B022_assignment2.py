...
class VCVS:
    ...
    def fillMatrix(self, G_matrix, nodes):
        ...
        if k != "GND" and vkl != "GND":
            G_matrix[k][vkl] += 1
            G_matrix[vkl][k] += 1

        if l != "GND" and vkl != "GND":
            G_matrix[l][vkl] += -1
            G_matrix[vkl][l] += -1

        if m != "GND" and vkl != "GND":
            G_matrix[vkl][m] += -self.gain

        if n != "GND" and vkl != "GND":
...
def getComponents(LinesTokens):
    ...
    for line in LinesTokens:
        ...
        if object_name[0] == "R":  # Resistor
            ...
            object = VoltageSource(name, n1, n2, value, VS_index)
            VS_index += 1
            ...
            object = VCVS(name, node1, node2, nc1, nc2, float(gain), VS_index)
            VS_index += 1
            ...
            print("Invalid component in input file !")
            exit()
...
def updateMatrix(G_matrix, I_vector, components, nodes):
    for object in components:
        if type(object).__name__ == R: # Resistor
            object.fillMatrix(G_matrix, nodes)

        elif type(object).__name__ == V: # Voltage source
            object.fillMatrix(G_matrix, I_vector, nodes)

        elif type(object).__name__ == I: # Current Source
            object.fillMatrix(I_vector, nodes)

        elif type(object).__name__ == C: # Capacitor
            object.fillMatrix(G_matrix, nodes)

        elif type(object).__name__ == L: # Inductor
            object.fillMatrix(G_matrix, nodes)

        elif type(object).__name__ == E: # VCVS
            object.fillMatrix(G_matrix, nodes)

        elif type(object).__name__ == G: # VCCS
            object.fillMatrix(G_matrix, nodes)
...
def printResult(V_vector, nodes, components):
    ...
    for object in components:
        if type(object).__name__ == V:
            print(
                f"I through {object.name}: {V_vector[len(nodes) - 1 + object.index]}",
                end="\n\n",
            )

        if type(object).__name__ == E:
            print(
                f"Current through {object.name}: {V_vector[len(nodes) - 1 + object.index]}"
            )
            pass
...
if __name__ == "__main__":
    ...
    try:
        with open(argv[1]) as f:
            ...
            components = getComponents(LinesTokens)
            ...
            VS_index = getVSCount(components)

            rows = node_count + VS_index - 1
            columns = rows
